...
pub async fn model_garden(server_config: &ServiceConfig) -> LoaderResult<ModelGarden>{
    ...
    let file_path = Path::new(&manifest_dir)
        .join("src/modules/loader/assets/iso639.json");
    let mut language_permutations: Vec<Vec<&String>> = server_config.server()
                                                                    .allowed_languages()
                                                                    .iter()
                                                                    .permutations(2)
                                                                    .collect();
    ...
    if let Some(langs) = deserialized_object {
       for (index,allowed_language) in language_permutations.iter().enumerate(){
            ...
            if index == 0{
                let source_formalized_name = langs[source_language]["name"].as_str().unwrap().to_string();
                let source_formalized_iso = langs[source_language]["639-1"].as_str().unwrap().to_string();
                let mut language = Language::new(source_formalized_name, source_formalized_iso);
                
                let target_formalized_name =langs[target_language]["name"].as_str().unwrap().to_string();
                let target_formalized_iso = langs[target_language]["639-1"].as_str().unwrap().to_string();

                let target_garden_language = TargetLanguage::new(target_formalized_name, target_formalized_iso);
            ...
            if index > 0  {
                ...
                if previous_language != source_language{
                    ...
                    let target_garden_language = TargetLanguage::new(target_formalized_name, target_formalized_iso);
                    model_garden.languages_mut().last_mut().unwrap().targets_mut().push(target_garden_language);
                }
...
mod test_loader {
    ...
    async fn test_model_garden() -> Result<(), anyhow::Error> {
        ...
        let test_data = model_garden(&service_config).await?;
